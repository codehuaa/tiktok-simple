...
func (dao *LikeDao) GetLikeVideoRelationByUserVideoID(userID int64, videoID int64) (*model.LikeVideo, error) {
	likeVideo := new(model.LikeVideo)
	if err := dao.First(&likeVideo, "user_id = ? and video_id = ?", userID, videoID).Error; err == nil {
		return likeVideo, nil
